...
```typescript
import { OFFICE_THEME } from "@/components/onlyoffice-embed-sdk";
...
await manager.setTheme(OFFICE_THEME.NIGHT); // 切换到现代深色
manager.destroy();                          // 销毁
```
...
```tsx
import { ONLYOFFICE_CONTAINER_CONFIG } from "@/components/onlyoffice-embed-sdk";
